    ...
    def parse(self) -> Dict[str, Plugin]:
        ...
        for plugin_elem in root.findall('.//plugin'):
            try:
                ...
                api_version = api_version_elem.text.strip() if api_version_elem is not None and api_version_elem.text else None
                
                source_elem = plugin_elem.find('source')
                source_url = source_elem.text.strip() if source_elem is not None and source_elem.text else None
                ...
                open_source = (open_source_elem is not None and 
                               open_source_elem.text and 
                               open_source_elem.text.strip().lower() == "yes")
