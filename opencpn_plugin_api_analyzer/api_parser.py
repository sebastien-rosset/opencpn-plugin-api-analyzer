...
class ApiHeaderParser:
    ...
    def _find_clang_lib_path(self) -> str:
        ...
        for path in possible_paths:
            if os.path.exists(os.path.join(path, 'libclang.so')) or \
               os.path.exists(os.path.join(path, 'libclang.dylib')):
                return path
        
        # If we couldn't find the library in common paths, try to use the one
        ...
        self.logger.warning(
            "Couldn't find libclang path. You may need to set it manually with "
            "clang.cindex.Config.set_library_path()"
        )
        
        # Try to continue anyway - maybe it's in a standard location
        return ""
    ...
    def _get_full_name(self, node) -> str:
        ...
        if (parent_name and 
            node.kind != clang.cindex.CursorKind.NAMESPACE and
