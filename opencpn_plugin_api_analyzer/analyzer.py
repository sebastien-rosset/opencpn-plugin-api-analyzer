...
class PluginAnalyzer:
    ...
    def _analyze_file_content(
        ...
        for i in range(0, len(symbol_names), chunk_size):
            ...
            pattern = r'\b(?:' + '|'.join(re.escape(s.split('::')[-1]) for s in chunk) + r')\b'
        ...
        for symbol_name in symbols_to_check:
            ...
            if re.search(pattern, content):
                ...
                if symbol.kind in ('CXX_METHOD', 'FUNCTION_DECL') and '::' in symbol_name:
                    class_name = symbol_name.split('::')[-2]
                    # Look for pattern like "obj.method(" or "Class::method("
                    method_pattern = r'(?:\b' + re.escape(class_name) + r'(?:::|\.)' + re.escape(short_name) + r'\s*\()'
                ...
                if symbol.kind in ('CLASS_DECL', 'STRUCT_DECL'):
                    ...
                    class_pattern = r'(?:\b' + re.escape(short_name) + r'\s*(?:\*|\&)?\s*\w+)'
                ...
                if symbol.kind == 'ENUM_CONSTANT_DECL' and '::' in symbol_name:
                    ...
                    enum_pattern = r'(?:\b' + re.escape(enum_name) + r'::' + re.escape(short_name) + r'\b)'
                    using_pattern = r'(?:using\s+(?:namespace\s+)?' + re.escape(enum_name) + r'\s*;.*\b' + re.escape(short_name) + r'\b)'
                    if re.search(enum_pattern, content) or re.search(using_pattern, content, re.DOTALL):
                        found_symbols.add(symbol_name)
                        continue
                
                # For macros and simple functions, just finding the name is usually enough
                if symbol.kind in ('MACRO_DEFINITION', 'FUNCTION_DECL') and '::' not in symbol_name:
                    found_symbols.add(symbol_name)
                    continue
    ...
    def analyze_plugins(
        ...
        self.logger.info(f"Found {len(api_symbols)} API symbols and {len(plugins)} plugins")
        
        # Filter plugins if requested
        if plugin_names:
            plugins = {name: plugin for name, plugin in plugins.items() if name in plugin_names}
        ...
        for name, plugin in plugins.items():
            ...
            if not plugin.source_repo or not plugin.api_version:
                self.logger.warning(f"Skipping plugin without source or API version: {name}")
                continue
        ...
        for api_version, api_plugins in plugins_by_api.items():
            ...
            for name, plugin in tqdm(api_plugins.items(), desc=f"Processing {api_version}"):
                # Clone repository
                repo_path = self.repo_handler.clone_repo(plugin.source_repo)
                if not repo_path:
                    self.logger.warning(f"Failed to clone repository for plugin: {name}")
                    continue
